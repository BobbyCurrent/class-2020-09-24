...
```{r}
sep_change <- inner_join(sep_old, sep_new, 
                         by = "id", 
                         suffix = c("_old", "_new")) %>%
  mutate(diff = u_grad_new - u_grad_old)

#This shows the largest increases
...
#This shows the largest decreases
...
decreasing <- sep_change %>%
  arrange(diff) %>%
  slice(1:5)


increasing <- sep_change %>%
  arrange(desc(diff)) %>%
  slice(1:5)

rbind(increasing, decreasing)

```
```{r}
combinetable <- rbind(increasing, decreasing) %>%
  ggplot(aes(x = diff, y = title_old))+
  geom_col()+
  labs(title = "Change in Course Enrollment at Harvard", x = "Change in Enrollment", y = "Course Name")
...
```{r}
#A table with no missing data, even the things that we could not reasonably know in real life.
```
